...
int nCoinbaseMaturity = COINBASE_MATURITY_JPKC;
...
const string strMessageMagic = "kagonmacoin Signed Message:\n";
...
bool LoadBlockIndex()
{
    ...
    printf("%s Network: genesis=0x%s nBitsLimit=0x%08x nBitsInitial=0x%08x nStakeMinAge=%d nCoinbaseMaturity=%d nModifierInterval=%d\n",
           fTestNet? "Test" : "kagonmacoin", hashGenesisBlock.ToString().substr(0, 20).c_str(), bnProofOfWorkLimit.GetCompact(), bnInitialHashTarget.GetCompact(), nStakeMinAge, nCoinbaseMaturity, nModifierInterval);
...
bool InitBlockIndex() {
	...
	///// ジェネシスブロック探査
        ...
        printf("kagonmacoin Genesis Block Found:\n");
        printf("genesis hash=%s\n", block.GetHash().ToString().c_str());
        printf("merkle root=%s\n", block.hashMerkleRoot.ToString().c_str());
        block.print();

        printf("End kagonmacoin Genesis Block\n");
...
bool CheckWork(CBlock* pblock, CWallet& wallet, CReserveKey& reservekey)
{
    ...
    if (hash > hashTarget && pblock->IsProofOfWork())
        return error("kagonmacoinMiner : proof-of-work not meeting target");

    //// debug print
    printf("kagonmacoinMiner:\n");
    ...
    // Found a solution
    {
        ...
        if (pblock->hashPrevBlock != hashBestChain)
            return error("kagonmacoinMiner : generated block is stale");
        ...
        if (!ProcessBlock(state, NULL, pblock))
            return error("kagonmacoinMiner : ProcessBlock, block not accepted");
    }
...
#endif
{
    ...
    try { ploop {
        ...
        printf("Running kagonmacoinMiner with %" PRIszu" transactions in block (%u bytes)\n", pblock->vtx.size(),
    ...
    {
        if(!fProofOfStake) printf("kagonmacoinMiner terminated\n");
        throw;
    }
}
