...
class CoinContainer
  def create(options = {})
    ...
    bash_cmd += "./kagonmacoind " + cmd_args.join(" ")

    if options[:remove_addr_after_shutdown]
      bash_cmd += "; rm -f /root/.kagonmacoin/testnet/peers.dat"
    end

    if options[:remove_wallet_after_shutdown]
      bash_cmd += "; rm /root/.kagonmacoin/testnet/wallet.dat"
    end
