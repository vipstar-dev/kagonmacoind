...
// Copyright (c) 2011-2018 The kagonmacoin developers
...
#else
    const char* pszModule = "kagonmacoin";
#endif
    if (pex)
        return strprintf(
...
boost::filesystem::path GetDefaultDataDir()
{
    ...
    // Windows < Vista: C:\Documents and Settings\Username\Application Data\kagonmacoin
    // Windows >= Vista: C:\Users\Username\AppData\Roaming\kagonmacoin
    // Mac: ~/Library/Application Support/kagonmacoin
    // Unix: ~/.kagonmacoin
#ifdef WIN32
    // Windows
    return GetSpecialFolderPath(CSIDL_APPDATA) / "kagonmacoin";
#else
    fs::path pathRet;
...
{
    namespace fs = boost::filesystem;
    // Windows < Vista: C:\Documents and Settings\Username\Application Data\JPKCoin
    // Windows >= Vista: C:\Users\Username\AppData\Roaming\JPKCoin
    // Mac: ~/Library/Application Support/JPKCoin
    // Unix: ~/.ppcoin
#ifdef WIN32
    // Windows
    return GetSpecialFolderPath(CSIDL_APPDATA) / "JPKCoin";
#else
    fs::path pathRet;
    char* pszHome = getenv("HOME");
    if (pszHome == NULL || strlen(pszHome) == 0)
...
{
    boost::filesystem::path pathConfigFile(GetArg("-conf", "kagonmacoin.conf"));
...
{
    boost::filesystem::path pathPidFile(GetArg("-pid", "kagonmacoind.pid"));
    ...
    if (vTimeOffsets.size() >= 5 && vTimeOffsets.size() % 2 == 1)
    {
        ...
        else
        {
            ...
            if (!fDone)
            {
                ...
                {
                    fDone = true;
                    string strMessage = _("Warning: Please check that your computer's date and time are correct! If your clock is wrong kagonmacoin will not work properly.");
...
std::string FormatSubVersion(const std::string& name, int nClientVersion, const std::vector<std::string>& comments)
{
    ...
    ss << "/";
    ss << "kagonmacoin:" << FormatVersion(PEERCOIN_VERSION);
    ss << "(" << CLIENT_BUILD << ")/";
    return ss.str();
}
